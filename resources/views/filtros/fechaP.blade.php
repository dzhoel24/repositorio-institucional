<x-app-main>
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 w-full gap-2 sm:gap-4">
        ...
        <div class="md:col-span-3 flex flex-col w-full px-4">
            <h2 class="text-2xl font-semibold py-2">Listar por fecha de Publicación</h2>
            ...
            <div class="py-2">
                {{-- ✅ CORREGIDO: Buscador específico por año --}}
                <x-search :parametro="'filtros'" :parametro2="'fechaP'" :descrip="'Introduce el año de publicación'" :text="'IR'" />
            </div>

            <div class="py-2 gap-4 w-full ">
                @foreach ($publi_fecha as $informe)
                    <x-card :parametro="'filtros'" :codigo="$informe->id" :image="$informe->ruta_caratula" :title="$informe->titulo"
                        :resumen="$informe->resumen" :autores="$informe->autores" :acceso="$informe->acceso" :action="'showFechaP'" />
                @endforeach
            </div>
