    ...
    <div class="overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
        ...
        <nav class="flex flex-col">
            <a href="{{ route('filtros.autores.index') }}"
                class="block text-sm text-slate-600 px-4 py-3 border-b border-slate-100 hover:bg-slate-50 transition-colors">
                Autores
            </a>
            ...
            <a href="{{ route('filtros.carreras.index') }}"
                class="block text-sm text-slate-600 px-4 py-3 hover:bg-slate-50 transition-colors">
        ...
        <nav class="flex flex-col">
            @forelse ($topAutors as $autor)
                <a href="{{ route('filtros.autores.informes', ['autor' => $autor['dni']]) }}"
                    ...
                    <span class="truncate pr-2 group-hover:text-indigo-600 transition-colors">
                        {{ $autor['apellidos'] }}, {{ $autor['nombre'] }}
                    </span>
                    <span
                        class="px-2 py-0.5 text-xs rounded-full bg-slate-100 text-slate-500 shrink-0 group-hover:bg-indigo-100 group-hover:text-indigo-600 transition-colors">
                        {{ $autor['count'] }}
                    </span>
                </a>
            @empty
                <div class="text-sm text-slate-500 px-4 py-6 text-center">
                    No hay autores destacados
                </div>
            @endforelse
            <a href="{{ route('filtros.autores.index') }}"
                class="block text-center text-sm text-indigo-600 px-4 py-3 hover:bg-indigo-50 transition-colors font-medium">
