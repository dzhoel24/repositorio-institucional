<div class="p-4 sm:p-5 md:p-6 bg-white rounded-xl shadow-sm border border-slate-200">
        ...
        <div class="md:col-span-5 lg:col-span-4 space-y-4">
            ...
            <div class="relative group rounded-xl overflow-hidden bg-slate-50 border border-slate-200 shadow-sm">
                ...
                <div class="absolute top-3 right-3">
                    <span
                        class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md text-[11px] sm:text-[11px] font-bold uppercase tracking-wider backdrop-blur-md shadow-sm text-white
                ...
                @if ($anio)
                    <div class="absolute bottom-3 left-3">
                        <span
                            class="inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-md bg-black/70 backdrop-blur-sm text-white text-[11px] sm:text-[11px] font-semibold">
                            <x-heroicon-s-calendar class="w-3.5 h-3.5 text-indigo-300" />
                            {{ $anio }}
                        </span>
                    </div>
                @endif
        ...
        <div class="md:col-span-7 lg:col-span-8 space-y-5">
            ...
            <div class="space-y-3">
                ...
                <div
                    ...
                    <x-heroicon-s-users class="w-4 h-4 text-indigo-500" />
                    <span class="font-semibold text-sm sm:text-sm">{{ $autores ?: 'Autor no especificado' }}</span>
                </div>
            </div>
            ...
            <div class="bg-slate-50/60 border-l-4 border-indigo-600 p-4 sm:p-5 rounded-r-xl">
                ...
                <p class="text-sm sm:text-sm font-medium text-slate-700 leading-relaxed text-left sm:text-justify">
                    {{ $resumen ?: 'Sin resumen disponible.' }}
                </p>
            </div>

            {{-- Metadatos --}}
            <div class="grid grid-cols-2 sm:flex sm:flex-wrap gap-3 pt-1">
                {{-- Tipo --}}
                <div class="flex items-center gap-2.5 bg-white border border-slate-200 p-2 rounded-lg shadow-sm">
                    <div class="p-1.5 bg-indigo-50 rounded-md">
                        <x-heroicon-s-document-text class="w-3.5 h-3.5 text-indigo-600" />
                    </div>
                    <div>
                        <p class="text-[9px] font-bold uppercase tracking-wider text-slate-400">Tipo</p>
                        <p class="text-sm font-bold text-slate-700">{{ $tipo ?? 'Documento' }}</p>
                    </div>
                </div>

                {{-- Condición --}}
                <div class="flex items-center gap-2.5 bg-white border border-slate-200 p-2 rounded-lg shadow-sm">
                    <div class="p-1.5 {{ $acceso === 'Publico' ? 'bg-emerald-50' : 'bg-rose-50' }} rounded-md">
                        @if ($acceso === 'Publico')
                            <x-heroicon-s-lock-open class="w-3.5 h-3.5 text-emerald-600" />
                        @else
                            <x-heroicon-s-lock-closed class="w-3.5 h-3.5 text-rose-600" />
                        @endif
                    </div>
                    <div>
                        <p class="text-[9px] font-bold uppercase tracking-wider text-slate-400">Condición</p>
                        <p
                            class="text-sm font-bold {{ $acceso === 'Publico' ? 'text-emerald-600' : 'text-rose-600' }}">
                ...
                @if ($anio)
                    <div class="flex items-center gap-2.5 bg-white border border-slate-200 p-2 rounded-lg shadow-sm">
                        <div class="p-1.5 bg-amber-50 rounded-md">
                            <x-heroicon-s-calendar class="w-3.5 h-3.5 text-amber-600" />
                        </div>
                        <div>
                            <p class="text-[9px] font-bold uppercase tracking-wider text-slate-400">Publicado</p>
                            <p class="text-sm font-bold text-slate-700">{{ $anio }}</p>
                        </div>
                    </div>
                @endif
            </div>
...
<script>
    function showToast(message) {
        ...
        toast.className =
            'fixed bottom-4 right-4 bg-slate-800 text-white text-sm px-4 py-2 rounded-lg shadow-lg z-50 opacity-0 transition-opacity duration-200';
