        ...
        <div
            ...
            <div
                class="flex-shrink-0 flex flex-col-reverse gap-2 border-t border-slate-100 bg-slate-50/50 px-4 py-3 
                        sm:flex-row sm:justify-end sm:px-6 sm:py-4 dark:border-slate-700 dark:bg-slate-800/50">
                <button type="button" data-tw-dismiss="modal"
                    class="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium text-slate-700 
                           transition-all duration-200 hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-800 
                           dark:text-slate-300 dark:hover:bg-slate-700">
                    Cancelar
                </button>
                <button type="submit" form="{{ $id }}-form"
                    class="rounded-lg bg-indigo-600 px-4 py-2 text-sm font-medium text-white 
                           transition-all duration-200 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2
                           dark:bg-indigo-500 dark:hover:bg-indigo-600">
                    <div class="flex items-center justify-center gap-2">
                        <x-dynamic-component :component="$buttonIcon" class="h-4 w-4" />
                        {{ $buttonText }}
                    </div>
                </button>
            </div>
