@props(['id', 'title', 'subtitle', 'size' => 'md', 'action'])
...
<div class="modal fixed inset-0 z-[999] hidden" id="{{ $id }}" role="dialog" aria-modal="true"
    ...
    <div class="relative flex items-center justify-center min-h-screen p-4 pointer-events-none">
        ...
        <div
            class="w-full {{ $modalSize }} max-h-[90vh] rounded-2xl bg-white dark:bg-slate-800 shadow-2xl overflow-hidden modal-animate pointer-events-auto flex flex-col">

            {{-- HEADER FIJO --}}
            <div
                class="flex-shrink-0 flex items-center justify-between border-b border-slate-100 dark:border-slate-700 bg-gradient-to-r from-slate-50 to-white dark:from-slate-800 dark:to-slate-800 px-4 py-4 sm:px-6 sm:py-5">
                <div class="flex items-center gap-3">
                    <div
                        class="flex h-8 w-8 sm:h-10 sm:w-10 items-center justify-center rounded-xl bg-emerald-100 dark:bg-emerald-500/20">
                ...
                <button type="button" data-tw-dismiss="modal"
                    class="flex h-7 w-7 sm:h-8 sm:w-8 items-center justify-center rounded-lg text-slate-400 transition-all duration-200 hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-700 dark:hover:text-white">
            ...
            {{-- CONTENIDO SCROLLEABLE (sin footer) --}}
