@foreach ($informes as $info)
    <x-admin.modal-edit id="edit-modal{{ $info->id }}" title="EDITAR PROYECTO" subtitle="{{ $info->titulo }}"
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2 lg:gap-0">
            ...
            <div class="space-y-3 lg:space-y-5 lg:border-r lg:border-slate-200 dark:lg:border-slate-700 p-4 sm:p-5">
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <div>
                            <label for="tipo_informe_{{ $info->id }}" class="mb-1.5 block text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-300">
                                Tipo de informe
                            </label>
                            <select id="tipo_informe_{{ $info->id }}" name="tipo_informe" required
                                ...
                                @foreach ($tipos as $tipo)
                                    <option value="{{ $tipo->id }}" {{ $info->tipo_informe_id == $tipo->id ? 'selected' : '' }}>
                                        {{ $tipo->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="caja-carrera hidden" data-id="{{ $info->id }}">
                            <label for="carrera_{{ $info->id }}" class="mb-1.5 block text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-300">
                                Carrera profesional
                            </label>
                            <select id="carrera_{{ $info->id }}" name="carrera"
                                ...
                                @foreach ($carreras as $carrera)
                                    <option value="{{ $carrera->id }}" {{ $info->carrera_id == $carrera->id ? 'selected' : '' }}>
                                        {{ $carrera->nombre }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="caja-modulo hidden" data-id="{{ $info->id }}">
                            <label for="modulo_{{ $info->id }}" class="mb-1.5 block text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-300">
                                Módulo académico
                            </label>
                            <select id="modulo_{{ $info->id }}" name="modulo"
                                ...
                                @foreach (['I', 'II', 'III', 'IV'] as $m)
                                    <option value="{{ $m }}" {{ $info->modulo == $m ? 'selected' : '' }}>{{ $m }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                ...
                <div>
                    ...
                    <div class="space-y-3">
                        <div>
                            <label for="titulo_{{ $info->id }}" class="mb-1.5 block text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-300">
                                Título del proyecto
                            </label>
                            <input id="titulo_{{ $info->id }}" type="text" name="titulo" required value="{{ $info->titulo }}"
                                   class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 sm:px-4 sm:py-3 text-sm text-slate-800 
                        ...
                        <div>
                            <label for="resumen_{{ $info->id }}" class="mb-1.5 block text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-300">
                                Resumen
                            </label>
                        ...
                        <div>
                            <label for="anio_{{ $info->id }}" class="mb-1.5 block text-xs sm:text-sm font-semibold text-slate-600 dark:text-slate-300">
                                Año
                            </label>
                            <input id="anio_{{ $info->id }}" type="number" name="anio" required value="{{ $info->anio }}"
                                   class="w-full rounded-xl border border-slate-200 bg-white px-3 py-2.5 sm:px-4 sm:py-3 text-sm text-slate-800 
                                          outline-none transition-all duration-200 placeholder:text-slate-400 
                                          focus:border-indigo-400 focus:ring-2 focus:ring-indigo-400/20 
                                          dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100 
            ...
            <div class="p-4 sm:p-5">
                ...
                <div class="space-y-4">
                    {{-- PDF --}}
                    <div class="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-4 dark:border-slate-700 dark:bg-slate-800/30">
                        <div class="mb-2 flex flex-wrap items-center gap-2">
                            ...
                            <label for="pdf_{{ $info->id }}" class="text-sm font-bold text-slate-600 dark:text-slate-300">Archivo PDF</label>
                        </div>
                    ...
                    <div class="rounded-xl border-2 border-dashed border-slate-200 bg-slate-50/50 p-4 dark:border-slate-700 dark:bg-slate-800/30">
                        <div class="mb-2 flex flex-wrap items-center gap-2">
                            ...
                            <label for="caratula_{{ $info->id }}" class="text-sm font-bold text-slate-600 dark:text-slate-300">Carátula</label>
                        </div>
                    ...
                    {{-- Preview --}}
                    <div class="rounded-xl border border-slate-200 bg-slate-50/50 dark:border-slate-700 dark:bg-slate-800/30">
                        <p class="border-b border-slate-200 px-4 py-2.5 text-[10px] font-black uppercase tracking-widest text-slate-400 dark:text-slate-500">
                            Vista previa
                        </p>
