<x-public.app-main title="Item Carreras">
    ...
    <div class="grid grid-cols-1 md:grid-cols-4 w-full gap-2 sm:gap-4">
        ...
        <div class="md:col-span-3 flex flex-col w-full px-4">
            ...
            <x-public.item class="col-span-2" :codigo="$item->codigo" :pdf="$item->ruta_pdf" :image="$item->ruta_caratula" :title="$item->titulo"
                :resumen="$item->resumen" :autores="$item->autores_formatted" :acceso="$item->acceso" :anio="$item->anio" :tipo="$item->tipo_informe" />
        </div>
    </div>
</x-public.app-main>
