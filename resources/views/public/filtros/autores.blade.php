        ...
        <main class="md:col-span-3 flex flex-col w-full px-0 md:px-4 space-y-5">
            ...
            <div
                ...
                <div class="bg-slate-100 rounded-full px-3 py-1 shrink-0 w-fit">
                    <span class="text-xs font-semibold text-slate-600">
                        <span class="text-indigo-600 font-bold">{{ $autores->total() }}</span> autores
                    </span>
                </div>
            </div>
            ...
            <div
                ...
                <div class="flex items-center justify-start sm:justify-end shrink-0">
                    <x-public.advanced-filter route="filtros.autores.index" :params="[]" defaultSort="asc"
            ...
            @if ($autores->isEmpty())
                <div class="text-center py-12 bg-slate-50/50 rounded-xl border border-dashed border-slate-200">
                    <x-heroicon-s-information-circle class="w-8 h-8 text-slate-300 mx-auto mb-3" />
                    <p class="text-sm font-medium text-slate-500 px-4">
                        No se encontraron autores con esos criterios.
                    </p>
                    @if (request('starts_with') || request('search'))
                        <a href="{{ route('filtros.autores.index') }}"
                            class="inline-flex items-center gap-2 mt-4 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-lg text-sm font-medium hover:bg-indigo-100 transition-colors">
                        ...
                        <a href="{{ route('filtros.autores.informes', ['autor' => $autor]) }}"
                            ...
                            <div class="flex items-center gap-2 shrink-0 ml-auto pl-2">
                                <span
                                    ...
                                    {{ $autor->informes_count === 1 ? 'doc' : 'docs' }}
                                </span>
